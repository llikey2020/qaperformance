...
kubectl exec alluxio-master-0 -c alluxio-master -- alluxio fs checkConsistency -r /${SPARK_WAREHOUSE} || true
kubectl exec alluxio-master-0 -c alluxio-master -- alluxio fs rm -RU /${SPARK_DEPENDENCY_DIR} || true
kubectl exec alluxio-master-0 -c alluxio-master -- alluxio fs checkConsistency -r /${SPARK_DEPENDENCY_DIR} || true
